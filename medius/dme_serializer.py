...
def _unknown_serialize(data: deque):
    # Eat all remaining bytes as raw and return a placeholder
    raw = ''.join(list(data))
    return {'raw': raw, 'packet_id': 'unknown'}
...
def dmetcp_serialize(data: bytes):
	...
	while len(data) != 0:
		...
		if dme_id in TcpSerializer:
			if dme_id in packets_both_tcp_and_udp:
				packets.append(TcpSerializer[dme_id].serialize('tcp', data))
			else:
				packets.append(TcpSerializer[dme_id].serialize(data))
		else:
			# Unknown packet - consume all remaining bytes to avoid infinite loop
			data.clear()
			packets.append({'packet_id': dme_id, 'unknown': True})
			break
	return packets
