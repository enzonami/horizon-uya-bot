...
class dm_crouch_body(State):
    ...
    def update(self):
        if not self.state_machine.game_state.nothing_to_do():
            if time.time() - self._entered_at > 1.5:
        ...
        if target_distance > 100:
            new_coord = self.state_machine.interpolate_toward(self.state_machine.game_state.map.path(self.state_machine.game_state.player.coord, self.state_machine.target, chargeboot=True))
